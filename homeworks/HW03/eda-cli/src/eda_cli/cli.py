...
def report(
    ...
    title: str = typer.Option("EDA-отчёт", help="Заголовок отчёта (будет в начале report.md)."),
    min_missing_share: float = typer.Option(0.3, help="Порог доли пропусков, выше которого колонка считается проблемной.",min=0.0,max=1.0,
    ),
) -> None:
    """
    ...
    #места(колонки), где проблемы по пропускам
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Есть колонки с нулевыми значениями (>50%): **{quality_flags['has_many_zero_values']}**\n")
