...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    Новые эвристики:
    1. has_constant_columns - есть колонки, где все значения одинаковые
    2. has_suspicious_id_duplicates - проверка уникальности идентификаторов
    3. has_many_zero_values - много нулевых значений в числовых колонках
    """
    flags: Dict[str, Any] = {}
    ...
    flags["has_constant_columns"] = any(
        col.unique == 1 and col.non_null > 0 
        for col in summary.columns
    )

    flags["has_suspicious_id_duplicates"] = any(
        any(id_word in col.name.lower() for id_word in ['id', 'user_id', 'client_id', 'customer_id'])
        and col.non_null > 0  # есть данные
        and col.unique < col.non_null  # не все значения уникальны
        for col in summary.columns
    )
    
    flags["has_many_zero_values"] = any(
        col.is_numeric  # числовая колонка
        and col.non_null > 0  # есть данные
        and col.min == 0  # минимальное значение = 0
        and col.mean is not None  # среднее посчитано
        and (
            col.max == 0  # все значения = 0
            or abs(col.mean) < 0.05 * abs(col.max)  # среднее близко к 0
        )
        for col in summary.columns
    )
    ...
    if flags["has_constant_columns"]:
        score -= 0.15  # константные колонки - серьезная проблема
    if flags["has_suspicious_id_duplicates"]:
        score -= 0.25  # дубликаты ID - очень серьезно
    if flags["has_many_zero_values"]:
        score -= 0.1   # много нулей - умеренная проблема
