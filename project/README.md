...
Проект посвящён построению сервиса рекомендаций рекламных каналов для сегментов аудитории.
Используются комбинация открытых датасетов и синтетические данные, модели CatBoost/XGBoost для прогнозирования и логистическая регрессия для интерпретации результатов.
Результат – REST API, который по профилю клиента возвращает ранжированный список рекомендованных каналов с вероятностями отклика и дашборд для анализа эффективности кампаний..
